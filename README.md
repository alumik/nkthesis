# nkthesis：南开大学硕士博士毕业论文模板（2026）

![version 2026.4.5](https://img.shields.io/badge/version-2026.4.5-blue)
...
常见问题及解答可在 ❓[常见问题及解答](https://github.com/alumik/nkthesis/wiki/%E5%B8%B8%E8%A7%81%E9%97%AE%E9%A2%98%E5%8F%8A%E8%A7%A3%E7%AD%94) 中查看。

模板存在的已知问题可在 ⚠️[已知问题或 BUG](https://github.com/alumik/nkthesis/wiki/%E5%B7%B2%E7%9F%A5%E9%97%AE%E9%A2%98%E6%88%96-BUG) 中查看。

![](https://pic1.imgdb.cn/item/69bbd388658eb5ba3dfa82fb.jpg)
...
> **更新记录与版本号说明：**
>
> - 版本号格式：`v<年份>.<主版本号>.<次版本号>`，例如 `v2026.4.0`。
>     - 年份：对应着当年的论文写作规范。
>     - 主版本号：当发生了**可能**需要修改论文内容的更新时，主版本号会增加。
>     - 次版本号：当发生了**不需要**修改论文内容的更新时，次版本号会增加。此时直接替换模板 `.cls` 文件即可，无需修改论文内容。
> - 更新记录说明：
>     - **[🚨不兼容变更]** 标记了需要重点关注的，可能需要修改论文内容的更新。
>     - 🔥 标记了以遵守论文写作规范为目的的格式修复或更新，建议立即更新。
>     - 其他未标注的更新属于可选更新。
...
| `uppercasehead`    | `false` | 当该选项存在时，将章节页眉转换为全大写。否则，保留原始大小写。                                          |
| `openright`        | `false` | 当该选项存在时，强制章节从右页开始。否则，允许章节从左页开始。                                          |
| `gbpub`            | `true`  | 当该选项存在时，或设置为真时，自动利用`[出版地不详]`、`[出版者不详]`、`[S.l.]`、`[s.n.]`等填补缺失的出版信息，否则取消相应项的输出。 |
| `frontmatterintoc` | `false` | 当该选项存在时，前置部分（如摘要、前言等）会被添加到目录中。否则，前置部分不会出现在目录中。            |
...
\nktset{
    ...
    论文完成时间 = {},
    论文编号 = {},
    非公开论文/申请密级 = 公开,  % 可选项：公开/限制/秘密/机密
    非公开论文/保密期限/起始日期 = {},
    非公开论文/保密期限/结束日期 = {},
    非公开论文/审批表编号 = {},
    非公开论文/批准日期 = {},
    原创性声明/电子签名路径 = {},
    原创性声明/电子签名宽度 = 7em,  % 默认为7em
    原创性声明/签字日期 = {},
    授权书/电子签名路径 = {},
    授权书/电子签名宽度 = 7em,  % 默认为7em
    授权书/签字日期 = {},
    学号 = {},
...
##### 2.5.1.1 引用格式一：使用原始 `\ref` 命令引用

使用该方式引用可输出最原始的数字标签，方便后续自定义引用文字。

| 引用对象 | 引用方式 | 引用样式（模式一，数字） | 引用样式（模式二，中文） |
| ------- | ------- | -------- | -------- |
| chapter | `\ref{chap:a}` | 1 | 一 |
| section | `\ref{sec:a}` | 1.1 | 一 |
| subsection | `\ref{subsec:a}` | 1.1.1 | 一 |
| subsubsection | `\ref{subsubsec:a}` | 1.1.1.1 | 一 |
| figure | `\ref{fig:a}` | 1.1 | 1.1 |
| subfigure | `\ref{subfig:a}` | 1.1（a） | 1.1（a） |
| table | `\ref{tab:a}` | 1.1 | 1.1 |
| equation | `\ref{eqn:a}` | （1.1） | （1.1） |

##### 2.5.1.2 引用格式二：使用 `\cref` 命令引用

`\cref` 命令可以输出格式化后的引用，对于章、节标题仅输出当前章、节编号，方便在文中使用类似“本章第二节”这样的表达。

| 引用对象 | 引用方式 | 引用样式（模式一，数字） | 引用样式（模式二，中文） |
| ------- | ------- | -------- | -------- |
| chapter | `\cref{chap:a}` | 第1章 | 第一章 |
| section | `\cref{sec:a}` | 第1.1节 | 第一节 |
| subsection | `\cref{subsec:a}` | 第1.1.1小节 | 第一小节 |
| subsubsection | `\cref{subsubsec:a}` | 第1.1.1.1子节 | 第一子节 |
| figure | `\cref{fig:a}` | 图1.1 | 图1.1 |
| subfigure | `\cref{subfig:a}` | 图1.1（a） | 图1.1（a） |
| table | `\cref{tab:a}` | 表1.1 | 表1.1 |
| equation | `\cref{eqn:a}` | 式（1.1） | 式（1.1） |

> [!CAUTION]
> 从 v2026.3.0 版本开始，不再支持使用 `\ref` 命令输出格式化后的引用。即不再支持直接输出“第一章第二节第三小节”这种格式。
